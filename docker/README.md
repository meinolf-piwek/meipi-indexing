...
(Exit-Code 1 bei Abweichungen). Tabellen, Datapool, ``serverpools``-Zeile und
Erstindexierung müssen vorher angelegt werden.
...
```bash
export IND_SERVER_NAME=lenox
meipi-index create-tables
# Alembic-Migrationen anwenden; serverpools-Zeile für jeden Pool anlegen, z. B.:
# INSERT INTO serverpools (server_name, pool_id, docroot) VALUES ('lenox', 1, '/data');
meipi-index read-files --pool-id 1 .
```

Im Container ist der Dateisystem-Root pro Pool in `serverpools.docroot` hinterlegt
(Seed bei Migration mit `IND_DOCROOT=/data`); der Host-Pfad kommt über `HOST_DATA_DIR`
...
| `IND_SERVER_NAME` | `lenox` | Server-Name für `serverpools` |
| `IND_DOCROOT` | `/data` | Nur für Alembic-Seed von `serverpools` |
| `IND_WATCH_POOL_ID` | `1` | Datapool für Service `watcher-1` |
| `IND_WATCH_PATH` | `.` | Pfad unter dem Pool-docroot |

...
def insert_pics_from_meta(pool):
    ...
    piclist = [dbpic_from_dbmeta(x,docroot=appconf.docroot) for x in metalist if appconf.get_ftype(x.suffix) == "pic"]
    with engine.Session() as session:
...
def read_no_heic():
    with engine.Session() as session:
        stmt = sa.select(DBPic.id, DBPic.path).where(
            DBPic.thumbarray == None          ).where(
            DBPic.suffix.not_in([".HEIC", ".heic"]) )
    return [(appconf.docroot+x.path, x.id) for x in session.execute(stmt)]
    
def read_pic_no_thumb():
    with engine.Session() as session:
        stmt = sa.select(DBPic.id, DBPic.path).where(
            DBPic.thumbarray == None)
    return [(appconf.docroot+x.path, x.id) for x in session.execute(stmt)]


def resize_pics(piclist, batch_size, pipe_batch_size, use_PIL):
    image_resizer = DALIImageResizer(pipe_batch_size=pipe_batch_size, num_threads=4, use_PIL=use_PIL)
